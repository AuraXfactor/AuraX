...
import { 
  getUniversalAuraFamMembers,
  getUniversalAuraFamStats,
  listenToUniversalAuraFam,
  removeUniversalAuraFamMember,
  searchUniversalAuraFamMembers,
  sortUniversalAuraFamMembers,
  UniversalAuraFamMember,
  UniversalAuraFamStats
} from '@/lib/universalAuraFamSystem';

interface UniversalAuraFamListProps {
  onMemberRemoved?: () => void;
}

export default function UniversalAuraFamList({ onMemberRemoved }: UniversalAuraFamListProps) {
  const { user } = useAuth();
  const router = useRouter();
  const [familyMembers, setFamilyMembers] = useState<UniversalAuraFamMember[]>([]);
  const [stats, setStats] = useState<UniversalAuraFamStats | null>(null);
  const [loading, setLoading] = useState(true);
  const [actionLoading, setActionLoading] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [sortBy, setSortBy] = useState<'name' | 'auraPoints' | 'recent' | 'online' | 'source'>('recent');

  const loadUniversalAuraFam = useCallback(async () => {
    if (!user) return;
    
    setLoading(true);
    try {
      console.log('🔄 Loading Universal Aura Fam...');
      const [members, familyStats] = await Promise.all([
        getUniversalAuraFamMembers(user.uid),
        getUniversalAuraFamStats(user.uid)
      ]);
      setFamilyMembers(members);
      setStats(familyStats);
      console.log('✅ Universal Aura Fam loaded:', members.length, 'members');
    } catch (error) {
      console.error('Error loading Universal Aura Fam:', error);
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  useEffect(() => {
    if (user) {
      loadUniversalAuraFam();
    }
  }, [user, loadUniversalAuraFam]);

  // Listen for Universal Aura Fam updates
  useEffect(() => {
    if (!user) return;

    const unsubscribe = listenToUniversalAuraFam(user.uid, (members, familyStats) => {
      setFamilyMembers(members);
      setStats(familyStats);
      setLoading(false);
    });
  ...
  // Listen for custom Universal Aura Fam update events
  useEffect(() => {
    const handleUniversalAuraFamUpdate = () => {
      if (user) {
        console.log('🔄 Universal Aura Fam update event received, refreshing...');
        loadUniversalAuraFam();
      }
    };

    window.addEventListener('universalAuraFamUpdated', handleUniversalAuraFamUpdate);
    return () => window.removeEventListener('universalAuraFamUpdated', handleUniversalAuraFamUpdate);
  }, [user, loadUniversalAuraFam]);

  const handleRemoveMember = async (member: UniversalAuraFamMember) => {
    ...
    try {
      await removeUniversalAuraFamMember({
  ...
  const handleStartChat = (memberId: string) => {
    console.log('🚀 Starting chat from Universal Aura Fam list', { memberId });
  ...
  const filteredAndSortedMembers = React.useMemo(() => {
    const filtered = searchUniversalAuraFamMembers(familyMembers, searchQuery);
    return sortUniversalAuraFamMembers(filtered, sortBy);
  ...
  return (
    <div className="space-y-6">
      {/* Universal Aura Fam Stats */}
      {stats && (
        <div className="bg-gradient-to-r from-purple-500 to-pink-500 rounded-2xl p-6 text-white">
          <h2 className="text-2xl font-bold mb-4">Universal Aura Fam Stats</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold">{stats.legacyMembers}</div>
              <div className="text-sm opacity-90">Legacy Fam</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold">{stats.newMembers}</div>
              <div className="text-sm opacity-90">New Fam</div>
            </div>
          </div>
          <div className="mt-4 text-center text-sm opacity-90">
            💡 This combines both old friends and newly accepted fam members
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 relative">
          <input
            ...
            placeholder="Search Universal Aura Fam..."
        ...
        >
          ...
          <option value="source">Source (Legacy/New)</option>
        </select>
      </div>
      ...
      {filteredAndSortedMembers.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold mb-2 text-gray-900 dark:text-white">
            {familyMembers.length === 0 ? 'No Universal Aura Fam yet' : 'No fam found'}
          </h3>
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        ...
                        {member.source === 'legacy' ? 'Legacy' : 'New'}
                      </span>
                    </div>
