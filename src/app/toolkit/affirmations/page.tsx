  ...
  return (
    <motion.div className="min-h-screen p-6 md:p-10 text-center" initial={prefersReducedMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      ...
      <div className="mt-6 max-w-md mx-auto flex items-center gap-2">
        <input value={text} onChange={(e) => setText(e.target.value)} placeholder="Add your own" className="flex-1 px-3 py-2 rounded-md bg-white/80 dark:bg-white/10 border border-white/20" />
        <button onClick={add} className="px-3 py-2 rounded-md bg-gradient-to-r from-purple-500 to-violet-500 text-white pressable">Add</button>
      </div>
      <div className="mt-6">
